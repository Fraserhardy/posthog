...
async def test_generate_ai_report_consent_revoked_aborts_and_auto_disables(team, user):
    ...
    assert result.recipient_results[0].error["type"] == AI_CONSENT_REVOKED_DISABLE_REASON.key
...
async def test_deliver_ai_subscription_missing_slack_integration_auto_disables(team, user):
    ...
    assert result.recipient_results[0].error["type"] == SLACK_DISCONNECTED_DISABLE_REASON.key
    await sync_to_async(sub.refresh_from_db)()
    assert sub.enabled is False
...
async def test_fetch_due_subscriptions_includes_ai_with_content_type(team, user):
    ...
    assert match.content_type == Subscription.ContentType.AI_PROMPT
