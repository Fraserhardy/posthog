...
| `limit`     | Max rows returned. Only HogQL endpoints support `offset`                                                                           |
| `offset`    | Skip rows. Only HogQL endpoints                                                                                                    |
| `refresh`   | `"blocking"` (wait for fresh data if cache miss), `"force"` (bypass cache), `"direct"` (skip cache write). Default is `"blocking"` |
...
For insight endpoints, the response shape differs depending on the query kind (`TrendsQuery`,
`FunnelsQuery`, etc.) — the OpenAPI spec captures the right shape for the current version.
...
| 429    | Rate limited (per-endpoint limits apply)                                       | Exponential backoff; cache responses client-side if possible                     |
...
1. Confirm endpoint name. If unknown, list with `endpoints-get-all`.
...
- **`?version=N` pins to a specific version.** Useful for callers that want stability across
  endpoint edits. Without it, the latest active version runs.
...
- **Rate limits are per-endpoint.** Heavy callers should batch where possible and back off on 429.
